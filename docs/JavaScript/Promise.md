...
- **Promise **是一个类，它的构造函数接收一个函数，函数有两个参数分别是`resolve` 和 `reject`
- **Promise** 可以链式调用，`.then(resolve,reject)` 也就是说`.then`其实也是个`Promise` 对象；
- **Promise** 中有个状态管理机制，其中有三个状态`pending`、`fulfilled`、`rejected`，初始化为`pending`，成功即`resolve`之后为`fulfilled`，失败`reject`之后状态置为`rejected`；
